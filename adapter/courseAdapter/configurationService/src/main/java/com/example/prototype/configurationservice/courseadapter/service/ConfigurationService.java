package com.example.prototype.configurationservice.courseadapter.service;

import com.example.prototype.configurationservice.courseadapter.DTO.CourseConfiguration;
import com.example.prototype.configurationservice.courseadapter.model.ConfigurationModel;
import com.example.prototype.configurationservice.courseadapter.repository.ConfigurationRepository;
...
public class ConfigurationService {
    ...
    // link another adapter with course timeline
    ...
    // unlink another adapter from course timeline
    ...
    public void setAdapterConfig(int courseId, String adapterIds) {
        ...
        if(!delAdapter.isEmpty()){
            String response = this.adapterWebClient.put().uri("/manageLink/"+courseId+"/Course Timeline/del/"+delAdapter.get(0)).retrieve().bodyToMono(String.class).block();
        } else if(!addAdapter.isEmpty()){
            String response = this.adapterWebClient.put().uri("/manageLink/"+courseId+"/Course Timeline/add/"+addAdapter.get(0)).retrieve().bodyToMono(String.class).block();
        }
        configurationRepository.setAdapterConfig(courseId, adapterIds);
    }

    // update adapter configuration with new values
    public void updateConfiguration(int courseId, CourseConfiguration configuration) {
        configurationRepository.updateConfig(courseId, configuration.isDisplayedDash(), configuration.getPeriodLength(), configuration.getPeriodStart(), configuration.isWarning());
    }
}
