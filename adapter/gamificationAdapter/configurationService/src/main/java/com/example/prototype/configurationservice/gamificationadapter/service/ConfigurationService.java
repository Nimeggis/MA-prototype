package com.example.prototype.configurationservice.gamificationadapter.service;

import com.example.prototype.configurationservice.gamificationadapter.DTO.GamificationConfiguration;
import com.example.prototype.configurationservice.gamificationadapter.model.ConfigurationModel;
import com.example.prototype.configurationservice.gamificationadapter.repository.ConfigurationRepository;
...
public class ConfigurationService {
    ...
    // link another adapter with gamification adapter
    ...
    // unlink another adapter from gamification adapter
    ...
    public void setAdapterConfig(int courseId, String adapterIds) {
        ...
        if(!delAdapter.isEmpty()){
            String response = this.adapterWebClient.put().uri("/manageLink/"+courseId+"/Gamification/del/"+delAdapter.get(0)).retrieve().bodyToMono(String.class).block();
        } else if(!addAdapter.isEmpty()){
            String response = this.adapterWebClient.put().uri("/manageLink/"+courseId+"/Gamification/add/"+addAdapter.get(0)).retrieve().bodyToMono(String.class).block();
        }
        configurationRepository.setAdapterConfig(courseId, adapterIds);
    }

    // update adapter configuration with new values
    public void updateConfiguration(int courseId, GamificationConfiguration configuration) {
        configurationRepository.updateConfig(courseId, configuration.isDisplayedDash(), configuration.getMilestone(), configuration.getPointReset(), configuration.isScoreboard());
    }
