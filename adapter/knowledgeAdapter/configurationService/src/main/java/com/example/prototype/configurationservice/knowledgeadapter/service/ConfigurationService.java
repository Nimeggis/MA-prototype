package com.example.prototype.configurationservice.knowledgeadapter.service;

import com.example.prototype.configurationservice.knowledgeadapter.DTO.KnowledgeConfiguration;
import com.example.prototype.configurationservice.knowledgeadapter.model.ConfigurationModel;
import com.example.prototype.configurationservice.knowledgeadapter.repository.ConfigurationRepository;
...
public class ConfigurationService {
    ...
    // link another adapter with knowledge progress adapter
    ...
    // unlink another adapter from knowledge progress adapter
    ...
    public void setAdapterConfig(int courseId, String adapterIds) {
        ...
        if(!delAdapter.isEmpty()){
            String response = this.adapterWebClient.put().uri("/manageLink/"+courseId+"/Knowledge Progress/del/"+delAdapter.get(0)).retrieve().bodyToMono(String.class).block();
        } else if(!addAdapter.isEmpty()){
            String response = this.adapterWebClient.put().uri("/manageLink/"+courseId+"/Knowledge Progress/add/"+addAdapter.get(0)).retrieve().bodyToMono(String.class).block();
        }
        configurationRepository.setAdapterConfig(courseId, adapterIds);
    }

    // update adapter configuration with new values
    public void updateConfiguration(int courseId, KnowledgeConfiguration configuration) {
        configurationRepository.updateConfig(courseId, configuration.isDisplayedDash(), configuration.getColorSuccess(), configuration.getColorInProgress(), configuration.getColorUpcoming());
    }
}
