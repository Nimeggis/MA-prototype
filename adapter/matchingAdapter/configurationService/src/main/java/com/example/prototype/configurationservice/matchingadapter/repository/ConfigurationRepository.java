package com.example.prototype.configurationservice.matchingadapter.repository;
...
public interface ConfigurationRepository extends JpaRepository<ConfigurationModel, Integer> {
    ...
    @Transactional
    @Query(value="INSERT INTO config (courseID, linked_adapter, is_displayed_dash, time_matching, layout, hint) VALUES (:courseId, '0', true, 60, 'vertical', true)",nativeQuery=true)
    ...
    @Modifying(clearAutomatically = true)
    @Transactional
    @Query(value="UPDATE config SET is_displayed_dash = :isDisplayed, time_matching = :timeMatching, layout = :layout, hint = :hint WHERE courseID = :courseId",nativeQuery=true)
    void updateConfig(Integer courseId, Boolean isDisplayed, Integer timeMatching, String layout, Boolean hint);
}
