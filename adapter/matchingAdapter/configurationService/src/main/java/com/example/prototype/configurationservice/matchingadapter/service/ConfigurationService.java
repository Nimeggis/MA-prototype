package com.example.prototype.configurationservice.matchingadapter.service;

import com.example.prototype.configurationservice.matchingadapter.DTO.MatchingConfiguration;
import com.example.prototype.configurationservice.matchingadapter.model.ConfigurationModel;
import com.example.prototype.configurationservice.matchingadapter.repository.ConfigurationRepository;
...
public class ConfigurationService {
    ...
    // link another adapter with matching exercise adapter
    ...
    // unlink another adapter from matching exercise adapter
    ...
    public void setAdapterConfig(int courseId, String adapterIds) {
        ...
        if(!delAdapter.isEmpty()){
            String response = this.adapterWebClient.put().uri("/manageLink/"+courseId+"/Matching Exercise/del/"+delAdapter.get(0)).retrieve().bodyToMono(String.class).block();
        } else if(!addAdapter.isEmpty()){
            String response = this.adapterWebClient.put().uri("/manageLink/"+courseId+"/Matching Exercise/add/"+addAdapter.get(0)).retrieve().bodyToMono(String.class).block();
        }
        configurationRepository.setAdapterConfig(courseId, adapterIds);
    }

    // update adapter configuration with new values
    public void updateConfiguration(int courseId, MatchingConfiguration configuration) {
        configurationRepository.updateConfig(courseId, configuration.isDisplayedDash(), configuration.getTimeMatching(), configuration.getLayout(), configuration.isHint());
    }
}
