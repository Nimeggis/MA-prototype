package com.example.prototype.configurationservice.flashcardadapter.repository;
...
public interface ConfigurationRepository extends JpaRepository<ConfigurationModel, Integer> {
    ...
    @Transactional
    @Query(value="INSERT INTO config (courseID, linked_adapter, is_displayed_dash, time_flashcard, days_repetition, hint) VALUES (:courseId, '0', true, 60, 5, true)",nativeQuery=true)
    ...
    @Modifying(clearAutomatically = true)
    @Transactional
    @Query(value="UPDATE config SET is_displayed_dash = :isDisplayed, time_flashcard = :timeFlashcard, days_repetition = :daysRepetition, hint = :hint WHERE courseID = :courseId",nativeQuery=true)
    void updateConfig(Integer courseId, Boolean isDisplayed, Integer timeFlashcard, Integer daysRepetition, Boolean hint);
}
