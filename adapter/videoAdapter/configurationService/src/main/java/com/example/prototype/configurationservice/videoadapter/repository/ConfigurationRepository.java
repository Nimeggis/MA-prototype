package com.example.prototype.configurationservice.videoadapter.repository;
...
public interface ConfigurationRepository extends JpaRepository<ConfigurationModel, Integer> {
    ...
    @Transactional
    @Query(value="INSERT INTO config (courseID, linked_adapter, is_displayed_dash, access_video, skip_video, pause_video) VALUES (:courseId, '0', true, true, false, true)",nativeQuery=true)
    ...
    @Modifying(clearAutomatically = true)
    @Transactional
    @Query(value="UPDATE config SET is_displayed_dash = :isDisplayed, access_video = :accessVideo, skip_video = :skipVideo, pause_video = :pauseVideo WHERE courseID = :courseId",nativeQuery=true)
    void updateConfig(Integer courseId, Boolean isDisplayed, Boolean accessVideo, Boolean skipVideo, Boolean pauseVideo);
}
