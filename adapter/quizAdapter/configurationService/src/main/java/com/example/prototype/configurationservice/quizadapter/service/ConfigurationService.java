package com.example.prototype.configurationservice.quizadapter.service;
...
import com.example.prototype.configurationservice.quizadapter.repository.ConfigurationRepository;
...
public class ConfigurationService {
    ...
    public void setAdapterConfig(int courseId, String adapterIds) {
        ...
        if(!delAdapter.isEmpty()){
            String response = this.adapterWebClient.put().uri("/manageLink/"+courseId+"/Quiz Upload/del/"+delAdapter.get(0)).retrieve().bodyToMono(String.class).block();
        } else if(!addAdapter.isEmpty()){
            String response = this.adapterWebClient.put().uri("/manageLink/"+courseId+"/Quiz Upload/add/"+addAdapter.get(0)).retrieve().bodyToMono(String.class).block();
        }
        configurationRepository.setAdapterConfig(courseId, adapterIds);
    }

    // update adapter configuration with new values
    public void updateConfiguration(int courseId, QuizConfiguration configuration) {
        configurationRepository.updateConfig(courseId, configuration.isDisplayedDash(), configuration.isAccessQuiz(), configuration.isSkipQuiz(), configuration.isHint());
    }
}
